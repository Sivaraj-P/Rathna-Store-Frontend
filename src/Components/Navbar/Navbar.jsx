...
import logo_black from '../../assets/rathna_store.png';
...
import { ShoppingCartRounded,Home ,Info,Category,Contacts,ListAlt,PersonAddOutlined} from '@mui/icons-material';
...
const Links = ({ drawer, setIsOpenDrawer, isOpenDrawer }) => {
    ...
    const pageLink = [new LinkClass(0, 'Home', <Home />), new LinkClass(1, 'About', <Info />), new LinkClass(2, 'Categories', <Category />),isUserLoggedIn && new LinkClass(3, 'Orders', <ListAlt />),isUserLoggedIn && new LinkClass(4, 'Account', <PersonAddOutlined />)].filter(Boolean);
    const componentsLink = [ new LinkClass('footer', 'Contact', <Contacts />)]
        ...
        : (< ul className={`flex p-0 sm:space-x-8 space-x-5' text-black`} >
            {
                pageLink.map(li => (
                    <Link to={`/${li.linkName.toLowerCase()}`} key={li.id}>
                        <li className='sm:text-base font-medium hover:text-gray-800 hover:scale-[0.99] text-sm'>
                        {/* {li.icon} */}
                            {li.linkName} 
                        </li>
                    </Link>
            ...
            {
                componentsLink.map((link, i) => (
                    <li key={i} className={`sm:text-base ${link.id !== 'footer' && pathname !== '/' && pathname !== '/home' ? 'hidden' : 'block'} font-medium hover:text-gray-800 transition-all duration-500 hover:scale-[0.99] text-sm cursor-pointer`}>
                        <ScrollToLink
                            to={link.id}
                            activeClass="active"
                            smooth={true}
...
export const userLoggedIn = JSON.parse(localStorage.getItem('userLoggedIn'));
